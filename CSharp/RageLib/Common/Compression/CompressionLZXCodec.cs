...
namespace RageLib.Common.Compression
{
    internal class CompressionLZXCodec : ICompressionCodec
    {
        ...
        public void Decompress(Stream source, Stream destination)
        {
            ...
            XboxLzxInterop.Initialize(17);

            uint bytesRead = 0;
            while (bytesRead < length)
            {
                if ((length - bytesRead) < 2 || source.Position >= source.Length)
                {
                    break;
                }
                ...
                if (inData.Length != inChunkSize)
                {
                    throw new EndOfStreamException("Could not read the full Xbox LZX chunk.");
                }
                ...
                try
                {
                    XboxLzxInterop.Decompress(inData, outData);
                }
                catch (DllNotFoundException ex)
                {
                    throw new Exception("Required Xbox LZX decoder DLL (xcompress_cpp.dll) was not found.", ex);
                }
                catch (EntryPointNotFoundException ex)
                {
                    throw new Exception("The Xbox LZX decoder DLL does not expose the expected functions.", ex);
                }
                catch
                {
                    throw new Exception("Could not decompress Xbox LZX resource chunk.");
                }
