...
namespace RageLib.Models.Resource
{
    ...
    class DrawableModel : PGBase, IFileAccess, IDataReader, IEmbeddedResourceReader, IDisposable
    {
        ...
        public new void Read(BinaryReader br)
        {
            ...
            for (int i = 0; i < levelOfDetailCount; i++)
            {
                ...
                catch
                {
                    ...
                    // PS3 .cdr drawables can expose the embedded texture dictionary even when
                    // geometry offsets do not match the PC layout expected by SparkIV.
                    ModelCollection = new PtrCollection<Model>[0];
                    break;
                }
            }
        }
        ...
        public void ReadEmbeddedResources(Stream systemMemory, Stream graphicsMemory, bool isBigEndian)
        {
            if (ShaderGroup.TextureDictionaryOffset != 0)
            {
                ...
                ShaderGroup.TextureDictionary.Open(systemMemory, graphicsMemory, isBigEndian);
            }
        }
